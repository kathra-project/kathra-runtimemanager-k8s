...
func (self ApplicationService) Add(namespace string, app *api.RuntimeAppInstance) (*api.RuntimeAppInstance, error) {
	var err = self.helmClt.InstallRelease(namespace, app.Name, app.CatalogEntry.ProviderID, app.CatalogEntry.Version, convertCatalogEntryArgumentsToStringMap(app.Parameters))
	if err != nil {
		log.Println(err)
		return nil, err
	}
